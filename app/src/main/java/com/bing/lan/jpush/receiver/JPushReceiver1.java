 ...
 * 2) 接收不到自定义消息
 */
public class JPushReceiver1 extends BroadcastReceiver {
    ...
    public JPushReceiver1() {
    }

    JPushListener mJPushListener;

    public JPushReceiver1(JPushListener jPushListener) {
        mJPushListener = jPushListener;
    }

    @Override
    public void onReceive(Context context, Intent intent) {
        if (mJPushListener != null) {
            mJPushListener.onReceive(context, intent);
        }
    ...
    private void onHandlerReceive(Context context, Intent intent) {
        ...
        Toast.makeText(context, printInfo(bundle), Toast.LENGTH_SHORT).show();
    ...
    private static String printInfo(Bundle bundle) {
        StringBuilder sb = new StringBuilder();
        for (String key : bundle.keySet()) {
            if (key.equals(JPushInterface.EXTRA_NOTIFICATION_ID)) {
                //sb.append("\n1--->key:" + key + ", value:" + bundle.getInt(key));
